...
const rows=SCIENCE_OBSERVATIONS.flatMap(activity=>activity.questions.map((q,i)=>{
 ...
 return {id:`${h.slice(0,8)}-${h.slice(8,12)}-4${h.slice(13,16)}-a${h.slice(17,20)}-${h.slice(20,32)}`,subject_id:'science',grade:activity.grade,difficulty:['K','1'].includes(activity.grade)?1:2,standard:null,skill:`${activity.grade}.observation`,prompt:q.prompt,choices:q.choices,answer_index:q.answer,explanation:q.explanation,xp:10,kind:'mcq',payload:{observation:activity.observation}};
}));
