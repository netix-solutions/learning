...
do $$declare s jsonb;begin
 ...
 if (s->>'balance')::int<>71 or jsonb_array_length(s->'cars')<>0 then raise exception 'Original-price refund failed';end if;
 s:=sell_reward('train','00000000-0000-4000-8000-000000003112');
 if (s->>'balance')::int<>71 then raise exception 'Double refund';end if;
 s:=buy_train_car('coal','00000000-0000-4000-8000-000000003112');
 if (s->>'balance')::int<>71 or jsonb_array_length(s->'cars')<>0 then raise exception 'Old purchase retry recreated item';end if;
 s:=sell_reward('engines','00000000-0000-4000-8000-000000003111');
 if s->>'activeEngine'<>'engine' or (s->>'balance')::int<>78 then raise exception 'Engine refund/fallback failed';end if;
 s:=buy_train_engine('passenger-engine','00000000-0000-4000-8000-000000003115');
 s:=sell_reward('engines','00000000-0000-4000-8000-000000003111');
 if s->>'activeEngine'<>'passenger-engine' or (s->>'balance')::int<>63 then raise exception 'Old refund affected repurchase';end if;
 ...
 if (s->>'balance')::int<>51 or jsonb_array_length(s->'dinosaurs')<>1 or jsonb_array_length(s->'treats')<>1 then raise exception 'Collection refunds failed';end if;
