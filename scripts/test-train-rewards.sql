...
select '00000000-0000-4000-8000-000000002501',q.id,q.subject_id,0,false,0 from questions q where grade='K' limit 13;
...
do $$declare s jsonb; a uuid:='00000000-0000-4000-8000-000000002511'; b uuid:='00000000-0000-4000-8000-000000002512';begin
 s:=get_my_train();if (s->>'balance')::int<>13 then raise exception 'Wrong starting balance';end if;
 perform set_reward_theme('train');
 s:=buy_train_car('passenger',a);if (s->>'balance')::int<>8 then raise exception 'Purchase balance wrong';end if;
 s:=buy_train_car('passenger',a);if (s->>'balance')::int<>8 or jsonb_array_length(s->'cars')<>1 then raise exception 'Retry charged twice';end if;
 s:=buy_train_car('cargo',b);if (s->>'balance')::int<>0 then raise exception 'Second balance wrong';end if;
