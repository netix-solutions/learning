...
for(const width of [320,390,768,1280]){
 ...
 assert.equal(await p.locator('.reward-item-toolbar').count(),0);await p.getByRole('button',{name:'Select Dining car 2',exact:true}).click();
 ...
 await p.getByRole('button',{name:'Select Coal car 2',exact:true}).click();assert.equal(await p.locator('.reward-item-toolbar').count(),1);
 ...
 for(const [theme,one,two,attr] of [['🦕 Dinosaurs','Triceratops','Stegosaurus','data-collected-dinosaur'],['🧁 Bakery','Sprinkle donut','Swirl cupcake','data-collected-treat']]){
  await p.getByRole('button',{name:theme,exact:true}).click();await p.getByRole('button',{name:`Buy ${one} for 5 tokens`,exact:true}).click();await p.getByRole('button',{name:`Buy ${two} for 8 tokens`,exact:true}).click();await p.getByRole('button',{name:`Select ${two} 2`,exact:true}).click();await p.getByRole('button',{name:`Move ${two} 2 earlier`,exact:true}).click();assert((await p.locator(`[${attr}]`).first().getByRole('group').getAttribute('aria-label')).includes(two));
  await p.getByRole('button',{name:`Select ${one} 2`,exact:true}).click();await p.getByRole('button',{name:`Sell back ${one} 2 for 2 tokens`,exact:true}).click();await p.getByRole('button',{name:`Confirm sell ${one} for 2 tokens`,exact:true}).click();assert.equal(await p.locator(`[${attr}]`).count(),1);
 }
