...
export function LearningRewards({grade,initialGarden=null,initialTrain,full=false,preview=false}:{grade:Grade|null;initialGarden?:GardenProgress|null;initialTrain?:TrainState;full?:boolean;preview?:boolean}) {
 ...
 return <div className="min-w-0"><div className="mt-6 flex flex-wrap items-center gap-2" role="group" aria-label="Choose your reward theme"><span className="mr-2 font-bold text-slate-700">My rewards</span>{(['garden','train'] as const).map(theme=><button key={theme} disabled={busy||!train} aria-pressed={train?.theme===theme} onClick={()=>choose(theme)} className={`min-h-12 rounded-xl border-2 px-4 py-3 font-bold ${train?.theme===theme?'border-sky-700 bg-sky-50 text-sky-900':'border-slate-200 bg-white text-slate-600'}`}>{theme==='garden'?'🌱 Garden':'🚂 Train'}</button>)}</div>
 ...
 {full&&train&&<p className="mt-4 text-sm text-slate-600">Choose either theme any time. Your flowers, tokens, and train cars stay saved.</p>}
 </div>;
}
