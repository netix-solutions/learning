...
import { LearningBakery } from '@/components/LearningBakery';
...
export function LearningRewards({grade,initialGarden=null,initialTrain,full=false,preview=false}:{grade:Grade|null;initialGarden?:GardenProgress|null;initialTrain?:TrainState;full?:boolean;preview?:boolean}) {
 ...
 return <div className="min-w-0" data-rewards-ready={ready}><div className="mt-6 grid grid-cols-2 items-stretch gap-2 sm:grid-cols-4" role="group" aria-label="Choose your reward theme"><span className="col-span-full mb-1 font-bold text-slate-700">My rewards</span>{(['garden','train','dinosaurs','bakery'] as const).map(theme=><button key={theme} disabled={!ready||busy||!train} aria-pressed={train?.theme===theme} onClick={()=>choose(theme)} className={`min-h-12 rounded-xl border-2 px-4 py-3 font-bold ${train?.theme===theme?'border-sky-700 bg-sky-50 text-sky-900':'border-slate-200 bg-white text-slate-600'}`}>{theme==='garden'?'🌱 Garden':theme==='train'?'🚂 Train':theme==='dinosaurs'?'🦕 Dinosaurs':'🧁 Bakery'}</button>)}</div>
 ...
 {train?.theme==='bakery'&&<LearningBakery initial={train} full={full} preview={preview} onChange={setTrain}/>}
 {train?.theme==='garden'&&<LearningGarden grade={grade} initial={initialGarden} full={full}/>}
 {full&&train&&<p className="mt-4 text-sm text-slate-600">Switch themes any time. Your flowers, train cars, dinosaurs, and bakery treats stay saved. All reward shops share your token balance.</p>}
 </div>;
}
