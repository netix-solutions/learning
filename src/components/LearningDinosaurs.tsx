...
export function LearningDinosaurs({ initial, full = false, preview = false, onChange }: { initial: TrainState; full?: boolean; preview?: boolean; onChange: (state: TrainState) => void }) {
  ...
  return <section aria-label="My dinosaur habitat" className="mt-5 min-w-0 overflow-hidden rounded-3xl border border-emerald-200 bg-[#fffdf6] shadow-sm">
    ...
    <div className="reward-scene-scroll" role="region" aria-label="Your collection. Scroll sideways to reach every item." tabIndex={full?0:undefined}><div role={full?undefined:"img"} aria-label={`Dinosaur habitat with ${collection.slice(shownPage * 6, shownPage * 6 + 6).length} dinosaurs`} className={`relative isolate aspect-[2/1] overflow-hidden ${full?"reward-interactive-scene":""}`}>
      ...
      {collection.slice(shownPage * 6, shownPage * 6 + 6).map((d, i) => <div key={d.id} data-collected-dinosaur className="absolute" style={{ left: `${3 + (i % 3) * 31}%`, bottom: full?(i < 3?'55%':'19%'):(i < 3?'24%':'2%'), width: '30%', height: full?'29%':'55%', zIndex: full?undefined:i < 3 ? 1 : 2 }}><Image src={`/images/dinosaurs/${d.dinosaurId}.webp`} alt="" fill sizes="(max-width: 768px) 32vw, 240px" className="pointer-events-none object-contain object-bottom" />{full&&<RewardItemControls state={state} kind="dinosaurs" purchase={d.id} preview={preview} busy={busy} onBusy={setBusy} onChange={next=>{setMessage('');setState(next);onChange(next);}} onMessage={setMessage}/>}</div>)}
    ...
    <div className="p-5 sm:p-7">
      {full&&collection.length>0&&<p className="mb-4 text-sm text-slate-600">Use the arrows on your items to rearrange them. The return arrow gives half your tokens back. Swipe to see more.</p>}
