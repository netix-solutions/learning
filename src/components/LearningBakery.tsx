...
export function LearningBakery({ initial, full = false, preview = false, onChange }: { initial: TrainState; full?: boolean; preview?: boolean; onChange: (state: TrainState) => void }) {
  ...
  return <section aria-label="My bakery shelf" className="mt-5 min-w-0 overflow-hidden rounded-3xl border border-rose-200 bg-[#fffaf4] shadow-sm">
    ...
    <div className="reward-scene-scroll" role="region" aria-label="Your collection. Scroll sideways to reach every item." tabIndex={full?0:undefined}><div role={full?undefined:"img"} aria-label={`Bakery display with ${collection.slice(shownPage * 6, shownPage * 6 + 6).length} treats`} className={`relative isolate aspect-[3/2] overflow-hidden ${full?"reward-interactive-scene":""}`}>
