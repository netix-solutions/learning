...
export function LearningBakery({ initial, full = false, preview = false, onChange }: { initial: TrainState; full?: boolean; preview?: boolean; onChange: (state: TrainState) => void }) {
  ...
  async function buy(id: string) {
    ...
    try {
      ...
      const next = preview ? { ...state, balance: state.balance - item.price, spent: state.spent + item.price, treats: [...collection, { id: request, treatId: item.id }] } : await buyBakeryTreat(id, request);
  ...
  return <section aria-label="My bakery shelf" className="mt-5 min-w-0 overflow-hidden rounded-3xl border border-rose-200 bg-[#fffaf4] shadow-sm">
    ...
    <div className="p-5 sm:p-7">
      ...
      {!full ? <Link href="/bakery" className="mt-5 inline-flex min-h-12 items-center rounded-xl bg-rose-800 px-5 py-3 font-bold text-white">Visit my bakery →</Link> : <><h3 className="mt-7 text-2xl font-bold text-slate-800">Pick something delicious</h3><p className="mt-1 text-slate-600">Choose a treat to add to your shelf. Collect your favorites again and again.</p><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">{(state.bakeryCatalog ?? []).map(d => <div key={d.id} className="flex min-w-0 flex-col rounded-2xl border border-rose-100 bg-white p-3"><div className="relative aspect-[3/2]"><Image src={`/images/bakery/${d.id}.webp`} alt="" fill sizes="(max-width: 640px) 160px, 230px" className="object-contain" /></div><h4 className="flex-1 break-words text-base font-bold text-slate-800">{d.name}</h4><p className="mt-2 font-bold text-amber-900">{d.price} tokens</p><button disabled={busy || state.balance < d.price} onClick={() => buy(d.id)} aria-label={`Buy ${d.name} for ${d.price} tokens`} className="mt-3 min-h-12 rounded-xl bg-rose-800 px-2 py-3 font-bold text-white disabled:bg-slate-100 disabled:text-slate-500">{state.balance < d.price ? `${d.price - state.balance} more to go` : 'Buy treat'}</button></div>)}</div><Link href="/practice/daily" className="mt-5 inline-flex min-h-12 items-center rounded-xl bg-rose-700 px-5 py-3 font-bold text-white">Practice for more tokens →</Link></>}
    </div>
  </section>;
}
